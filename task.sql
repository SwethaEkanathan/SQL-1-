...
SELECT film_id, COUNT(*) as film_count FROM inventory GROUP BY film_id ORDER BY film_count DESC LIMIT 1;
...
SELECT COUNT(DISTINCT district) AS distinct_district_count FROM address;
...
SELECT COUNT(*) FROM (SELECT COUNT(payment.rental_id) AS rental_count
FROM payment
WHERE payment.customer_id BETWEEN 380 AND 430
GROUP BY payment.amount
HAVING COUNT(payment.rental_id) > 250) AS counts;
